...
function applyBannerSlides(doc, desktopSources = [], mobileSources = []) {
    ...
    const slidesContainer = doc.getElementById('bannerSlides') || 
                           ...
                           doc.querySelector('.bannerTrack5') ||
                           doc.querySelector('.bannerTrack7');
    ...
    const isTemplate5Carousel = slidesContainer.classList && slidesContainer.classList.contains('bannerTrack5');

    const isTemplate7Carousel = slidesContainer.classList && slidesContainer.classList.contains("bannerTrack7");
    console.log(isTemplate7Carousel, "template-7");
    ...
    for (let i = 0; i < totalSlides; i++) {
        ...
        if (isTemplate3Carousel) {
            ...
            // Template-4 structure: bannerImageContainer.bannerSlide
            const slide = doc.createElement('div');
            slide.className = 'banner bannerTrack5';
            // if (i === 0) {
            //     slide.classList.add('is-active');
            // }
            ...
            img.setAttribute('height', '1125');
            img.setAttribute('sizes', '(max-width: 768px) 100vw, 75vw');
            img.setAttribute('fetchpriorty', i === 0 ? 'high' : 'low' );
            picture.appendChild(img);
        ...
        } else if (isTemplate7Carousel) {
            console.log("template-7",i)
            // Template-4 structure: bannerImageContainer.bannerSlide
            const slide = doc.createElement('div');
            slide.className = 'bannerCard';
            // if (i === 0) {
            //     slide.classList.add('is-active');
            // }
        ...
        if (isTemplate7Carousel && i === 0) {
            const bannersticky = doc.querySelector(".bannersticky");
            doc.querySelector(".bannersticky img").remove();
            const img = doc.createElement('img');
            img.setAttribute('alt', "highlight banner");
            img.setAttribute('loading','lazy');
            img.setAttribute('src',desktopSrc);
            bannersticky.appendChild(img);
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeTemplates().then(() => loadSavedState());
    ...
    async function embedImagesInPreview(html) {
        ...
        if ((bannerDesktopUrls.length || bannerMobileUrls.length) && (imageMapping.banner || imageMapping.mobBanner)) {
            ...
            const slidesContainer = doc.getElementById('bannerSlides') || 
                                   doc.querySelector('.carouselBannerTrack') ||
                                   doc.querySelector('.bannerSlidesContainer') ||
                                   doc.querySelector('.bannerContainer .bannerSlide');
        ...
        if (imageDataUrls['vtour'] && imageMapping.vtour) {
            ...
            const virtualTourSection = doc.querySelector('#virtual1, .virtualtourContainer, .virtualtourWrapper, .virtualTourSection, #virtualTourSite, #projectWalkthorugh');
        ...
        if (imageDataUrls['amenities'] && imageMapping.amenities) {
            ...
            if (amenitiesContainer) {
                ...
                const slideClass = isTemplate3Amenities ? 'amenities-slide' : (selectedTemplate === 'template-7' ? 'amenitiesCard' : 'amenitiesSlide');
                const imageWrapperClass = isTemplate3Amenities ? '' : 'amenitiesImageContainer';
                const imageClass = isTemplate3Amenities ? 'amenities-image' : 'amenitiesImage';
                const overlayClass = isTemplate3Amenities ? 'amenities-text-overlay' : (selectedTemplate === 'template-7'? 'amenities-description' : 'amenitiesTextOverlay');
                ...
                const count = rebuildCarouselSlides(amenitiesContainer, sources, {
                    ...
                    imageWrapperClass: selectedTemplate === 'template-7' ? '' : imageWrapperClass,
        ...
        if (imageDataUrls['gallery'] && imageMapping.gallery) {
            ...
            if (galleryContainer) {
                ...
                const slideClass = isCarouselTrack ? 'gallery-carousel-slide' : (selectedTemplate === 'template-7' ? 'galleryCard' : 'gallerySlide');
                const imageClass = isCarouselTrack ? 'gallery-carousel-image' : 'galleryImage';
                const imageWrapperClass = 'galleryImageContainer';
                const overlayClass = 'galleryTextOverlay';
            ...
            // Handle standalone gallery images with data-field="gallery" that are NOT in carousel containers
            // (e.g., template-2 overviewImageSection gallery image)
            const standaloneGalleryImages = doc.querySelectorAll('img[data-field="gallery"]');
            const galleryUrls = (imageDataUrls['gallery'] || []).filter(Boolean);
            standaloneGalleryImages.forEach((img, idx) => {
                // Skip if this image is inside a gallery carousel/track container
                const isInCarousel = img.closest('#galleryCarouselTrack, #galleryTrack, .gallery-carousel-track, .gallery-track, .galleryCarousel, .gallery-wrapper');
                if (!isInCarousel && galleryUrls[idx]) {
                    img.src = galleryUrls[idx];
                    img.setAttribute('data-src', galleryUrls[idx]);
                }
            });
            
            // Handle images in overviewImageSection that should use gallery images (template-4)
            // These images might not have data-field="gallery" but should still use gallery uploads
            const overviewImageSection = doc.querySelector('.overviewImageSection');
            if (overviewImageSection && galleryUrls.length > 0) {
                const overviewImg = overviewImageSection.querySelector('img.overviewImage');
                if (overviewImg && !overviewImg.hasAttribute('data-field') || overviewImg.getAttribute('data-field') !== 'gallery') {
                    // Use first gallery image for overviewImageSection if no gallery images already processed
                    const isAlreadyProcessed = overviewImg.closest('#galleryCarouselTrack, #galleryTrack, .gallery-carousel-track, .gallery-track, .galleryCarousel, .gallery-wrapper');
                    if (!isAlreadyProcessed) {
                        // Check if this image hasn't been updated by another handler
                        const src = overviewImg.getAttribute('src') || '';
                        // If it looks like a gallery/banner placeholder image, use first gallery upload
                        if (src.includes('gallery') || src.includes('banner') || src.includes('g') || src.includes('banner.webp')) {
                            overviewImg.src = galleryUrls[0];
                            overviewImg.setAttribute('data-src', galleryUrls[0]);
                        }
                    }
                }
            }
        ...
        // Update QR images using DOM - create dynamically
        console.log(imageDataUrls['qr'] ,"qr related data", imageMapping.qr)
        if (!imageDataUrls['qr']){
            const qrcontainers = doc.querySelectorAll('.aboutSectionQr, .aboutSection row mob-center, .footer-qr-section');
        ...
        if (imageDataUrls['qr'] && imageMapping.qr) {
            ...
            const pattern = imageMapping.qr.pattern || '{index}.webp';
            
            // Find QR container - try multiple selectors
            const qrContainer = doc.querySelector('.aboutSectionQr');
            const aboutSection = doc.querySelector('.aboutSection');
            const footersection = doc.querySelector(".footer-qr-section");
            
            if (qrContainer || aboutSection || footersection) {
                // Find all rera-img containers (QR image containers) - these are the actual QR containers
                const reraImgContainers = (qrContainer || aboutSection || footersection).querySelectorAll('.rera-img, .footer-qr-item');
                ...
                if (reraImgContainers.length > 0) {
                    ...
                    imageDataUrls['qr'].forEach((dataURL, index) => {
                        ...
                        if (!reraContainer && footersection) {
                            // Create new container structure if needed
const rowContainer = footersection.querySelector('.footer-qr-grid');
                            if (rowContainer) {
                                // const colDiv = doc.createElement('div');
                                // colDiv.className = 'col-lg-2 col-sm-3';
                                reraContainer = doc.createElement('div');
                                reraContainer.className = 'footer-qr-item';
                                // colDiv.appendChild(reraContainer);
                                rowContainer.appendChild(reraContainer);
                            }
                        }
                        ...
                        if (reraContainer) {
                            ...
                            // Set appropriate class for footer QR images
                            if (reraContainer.classList.contains('footer-qr-item')) {
                                qrImg.className = 'footer-qr-code';
                            }
                    ...
                    if (reraImgContainers.length > imageDataUrls['qr'].length) {
                        for (let i = imageDataUrls['qr'].length; i < reraImgContainers.length; i++) {
                            ...
                            const colDiv = container.closest('.col-lg-2, .col-sm-3, .footer-qr-item');
                            if (colDiv) {
                                colDiv.remove();
                        } else {
                                container.remove();
                    ...
                    // No rera-img containers found, try to find QR images directly (but exclude logo and RERA panel)
                    const qrImages = Array.from((qrContainer || aboutSection || footersection).querySelectorAll('img[src*="qr"], img[src*="QR"], img[data-field="qr"]')).filter(img => {
                        // Exclude logo images - only find actual QR images
                        const isLogo = img.closest('.aboutLogo, .logo-container, header, .header');
                        // Exclude RERA panel QR images - only include footer QR images
                        const isReraPanel = img.closest('.qr-section') && !img.closest('.footer-qr-item');
                        const isFooterQr = img.closest('.footer-qr-item');
                        return !isLogo && !isReraPanel && (isFooterQr || !img.closest('.qr-section'));
                    });
                    ...
                    if (uploadedQr.length > qrImages.length) {
                        // Try to add to row container first (template-3 structure)
                        if (aboutSection) {
                            const rowContainer = aboutSection.querySelector('.row.mob-center, .row');
                            if (rowContainer) {
                        ...
                        // If no row container, add directly to qrContainer (template-1, template-2 structure)
                        if (qrContainer && (!aboutSection || !aboutSection.querySelector('.row.mob-center, .row'))) {
                            for (let i = qrImages.length; i < uploadedQr.length; i++) {
                                const qrImg = doc.createElement('img');
                                qrImg.src = uploadedQr[i];
                                qrImg.alt = `QR Code ${i + 1}`;
                                qrImg.setAttribute('data-src', uploadedQr[i]);
                                qrImg.setAttribute('data-field', 'qr');
                                qrImg.setAttribute('loading', 'lazy');
                                qrImg.setAttribute('height', '120');
                                // Add spacing between QR images
                                if (i > 0) {
                                    qrImg.style.marginLeft = '10px';
                                }
...
const rowContainer = footersection.querySelector('.footer-qr-grid');
                            if (rowContainer) {
                                for (let i = qrImages.length; i < uploadedQr.length; i++) {
                                    ...
                                    // colDiv.className = 'col-lg-2 col-sm-3';
                                    const reraDiv = doc.createElement('div');
                                    reraDiv.className = 'footer-qr-item';
                                    const qrImg = doc.createElement('img');
                                    qrImg.src = uploadedQr[i];
                                    qrImg.alt = `MahaRERA QR Code ${i + 1}`;
                                    qrImg.setAttribute('data-src', uploadedQr[i]);
                                    ...
                                    reraDiv.appendChild(qrImg);
                                    // colDiv.appendChild(reraDiv);
                                    rowContainer.appendChild(reraDiv);
                                }
                            }
                        }
                    }
                ...
                // Fallback: find QR images without container (but exclude header/logo and RERA panel)
                const qrImages = Array.from(doc.querySelectorAll('img[src*="qr"], img[src*="QR"], img[data-field="qr"]')).filter(img => {
                    ...
                    const isReraPanel = img.closest('.qr-section') && !img.closest('.footer-qr-item');
                    if (isReraPanel) {
                        return false; // Skip RERA panel QR
                    }
                    return (src && (src.includes('qr') || src.includes('QR'))) || img.getAttribute('data-field') === 'qr';
                });
                ...
                // Add new QR images if we have more uploads than existing (fallback for templates without rera-img containers)
                if (uploadedQr.length > qrImages.length && qrContainer) {
                    for (let i = qrImages.length; i < uploadedQr.length; i++) {
                        const qrImg = doc.createElement('img');
                        qrImg.src = uploadedQr[i];
                        qrImg.alt = `QR Code ${i + 1}`;
    ...
    async function copyTemplateAssets(zip) {
        ...
        // Add font files for template-4
        if (selectedTemplate === 'template-4') {
            allTemplateFiles.push(
                'assets/fonts/futura_book_bt.woff2',
        ...
        if (selectedTemplate === 'template-2') {
            allTemplateFiles.push(
                'assets/fonts/FuturaBT-Book.otf',
                'assets/fonts/historypro-two.ttf',
            );
        }

        if (selectedTemplate === 'template-5') {
            allTemplateFiles.push(
                'assets/fonts/FuturaBkBTBook.ttf',
                'assets/images/icons/chevron-left.svg',
                'assets/images/icons/chevron-right.svg',
                'assets/images/icons/fa-close.svg',
                'assets/css/aos.css',
            );
        }
        ...
        if (selectedTemplate === "template-7") {
            allTemplateFiles.push(
                ...
                'assets/images/icons/location.webp'
            )
        }
        ...
        if (availableTemplates.length === 0) {
            ...
            selector.innerHTML = `
                <div class="loading-templates">
                    ...
                    <p style="margin-top: 10px; font-size: 14px;">Add template folders (template-1, template-2, etc.) or update templates.json</p>
    ...
    async function generateMicrositeHTML() {
        ...
        console.log("starting offer section");
        const offersContainer = doc.querySelector('.bannerOffer, .offerBoxContainer, .bannerOfferWrapper, .offer-item');
        if (offersContainer) {
            ...
            const offerTemplate = offersContainer.querySelector('.offerText[data-field^="offer_"]') || 
                                 ...
                                 offersContainer.querySelector('.bannerOfferItem') ||
                                 offersContainer.querySelector('.offer-list');
        ...
        const highlightsList = doc.querySelector('.highlightsList, .highlight-list, .highlightpoints');
        if (highlightsList) {
        ...
        //for template-5
        if (selectedTemplate == 'template-5') {
        const priceContainer = doc.querySelector('.price-listContainer');
        if (priceContainer) {
            // Find first price card as template
            const priceCardTemplate = priceContainer.querySelector('.price-itemContainer[data-field*="price_"]') ||
                                    priceContainer.querySelectorAll('.price-itemContainer')[1];
        ...
        //for template-7
        const priceCardContainer = doc.querySelector('.floorCaurosel');
        if (priceCardContainer) {
            const priceContainer = doc.querySelector('.floorSlide');
        if (priceContainer) {
            console.log("inside price section for template-7");
            // Find first price card as template
            const priceCardTemplate = priceContainer.querySelector('.floorCard[data-field*="price_"]') ||
                                    priceContainer.querySelectorAll('.floorCard')[0];
            
            if (priceCardTemplate) {
                console.log("fetching price card");
                // Collect all price card data
                const priceCards = [];
                for (let i = 1; i <= 10; i++) {
                    const typeField = `price_${i}_type`;
                ...
                // Remove all existing price cards (except template)
                const existingCards = priceContainer.querySelectorAll('.floorCard');
                existingCards.forEach(card => {
                    if (card !== priceCardTemplate) {
                        card.remove();
                    }
        ...
        if (mapRightContainer) {
            ...
            if (dropdownTemplate) {
                ...
                //Handle location dropdown for template-5
        if (selectedTemplate == "template-5"){
            const mapContainer = doc.querySelector('.location-accordionContainer');
        if (mapContainer) {
            // Find first dropdown as template
            const dropdownTemplate = mapContainer.querySelector('.location-accordionDropdownContainer');
            ...
            if (dropdownTemplate) {
                ...
                // Remove all existing dropdowns (except template)
                const existingDropdowns = mapContainer.querySelectorAll('.location-accordionDropdownContainer');
                existingDropdowns.forEach(dropdown => {
                    if (dropdown !== dropdownTemplate) {
                        dropdown.remove();
                    }
                ...
                // Get template connectivity item BEFORE we modify the template
                const itemTemplate = dropdownTemplate.querySelector('.location-accordion-item');
                ...
                dropdownsData.forEach((dropdownData, idx) => {
                    ...
                    if (isTemplate) {
                        ...
                        // Keep "active" class for first dropdown only
                        const btn = dropdownEl.querySelector('.location-accordionBtn');
                        const content = dropdownEl.querySelector('.location-accordionContent');
                        const arrow = dropdownEl.querySelector('.location-accordionArrow');
                        // if (btn && idx === 0) {
                        //     btn.classList.add('active');
                        // } else if (btn) {
                        //     btn.classList.remove('active');
                        // }
                        if (content && idx === 0) {
                            content.classList.add('location-accordionContent-visible');
                        } else if (content) {
                            content.classList.remove('location-accordionContent-visible');
                        }
                        if (arrow && idx === 0) {
                            arrow.classList.add('location-arrowActive');
                        } else if (arrow) {
                            arrow.classList.remove('location-arrowActive');
                        }
                        
                        // Clear existing items from template too
                        if (content) {
                            const existingItems = content.querySelectorAll('.location-accordion-item');
                            existingItems.forEach(item => item.remove());
                        }
                    } else {
                        // Create new dropdown
                        ...
                        const arrow = dropdownEl.querySelector('.location-accordionArrow');
                        // if (btn) btn.classList.remove('active');
                        if (content) content.classList.remove('location-accordionContent-visible');
                        if (arrow) arrow.classList.remove('location-arrowActive');
                        ...
                        mapContainer.appendChild(dropdownEl);
                    }
                    
                    // Get dropdown content container
                    let contentEl = dropdownEl.querySelector('.location-accordionContent');
                    if (!contentEl) {
                        contentEl = doc.createElement('div');
                        contentEl.className = 'location-accordionContent';
                        const buttonEl = dropdownEl.querySelector('.location-accordionBtn');
                        if (buttonEl && buttonEl.nextSibling) {
                            dropdownEl.insertBefore(contentEl, buttonEl.nextSibling);
                        } else {
                            dropdownEl.appendChild(contentEl);
                    ...
                    // Clear existing connectivity items
                    const existingItems = contentEl.querySelectorAll('.location-accordion-item');
                    existingItems.forEach(item => item.remove());
                    ...
                    if (itemTemplate && dropdownData.items.length > 0) {
                        dropdownData.items.forEach(item => {
                            ...
                            // Update location field
                            const locationEl = clone.querySelector('.location-itemContent');
                            if (locationEl) {
                                locationEl.setAttribute('data-field', item.valueField);
                                locationEl.textContent = item.value || '';
                            }
                            ...
                            // Update distance/time field
                            const timeEl = clone.querySelector('.location-itemTime');
                            if (timeEl) {
                                timeEl.setAttribute('data-field', item.distanceField);
                                timeEl.textContent = item.distance || '';
                            }
                ...
                // Ensure map-cta button is at the bottom of map-right container
                const mapCtaButton = mapContainer.querySelector('.location-BtnContainer');
                if (mapCtaButton) {
                    // Remove it from current position
                    mapCtaButton.remove();
                    // Append it to the bottom
                    mapContainer.appendChild(mapCtaButton);
                }
        ...
        if (accordionWrapper) {
            const dropdownTemplate = accordionWrapper.querySelector('.accordion-contentWrapper');
            ...
            if (dropdownTemplate) {
                ...
                // Remove all existing dropdowns (except template)
                const existingDropdowns = accordionWrapper.querySelectorAll('.accordion-contentWrapper');
                existingDropdowns.forEach(dropdown => {
                    if (dropdown !== dropdownTemplate) {
                        dropdown.remove();
                    }
                ...
                dropdownsData.forEach((dropdownData, idx) => {
                    ...
                    if (isTemplate) {
                        ...
                        // Keep "active" class for first dropdown only
                        const btn = dropdownEl.querySelector('.mobile-accordionBtn');
                        const content = dropdownEl.querySelector('.accordion-content ul');
                        const arrow = dropdownEl.querySelector('.accordion-arrow');
                        if (btn && idx === 0) {
                        ...
                        // Remove "active" classes from cloned dropdowns
                        const btn = dropdownEl.querySelector('.mobile-accordionBtn');
                        const content = dropdownEl.querySelector('.accordion-content');
                        const arrow = dropdownEl.querySelector('.accordion-arrow');
                        if (btn) btn.classList.remove('accordion-active');
                        if (content) content.classList.remove('contentactive');
                        if (arrow) arrow.classList.remove('accordion-arrow-active');
                        ...
                        if (titleSpan) {
                            ...
                            // Find span in button
                            if (btn) {
                                const span = btn.querySelector('span:not(.accordion-arrow)');
                                if (span) {
                                    span.setAttribute('data-field', dropdownData.connectivityField);
                                    span.textContent = dropdownData.title;
                                }
                        ...
                        accordionWrapper.appendChild(dropdownEl);
                    }
                    
                    // Get dropdown content container
                    let contentEl = dropdownEl.querySelector('.accordion-content');
                    if (!contentEl) {
                        contentEl = doc.createElement('div');
                        contentEl.className = 'accordion-content';
                        const buttonEl = dropdownEl.querySelector('.mobile-accordionBtn');
                        if (buttonEl && buttonEl.nextSibling) {
                            dropdownEl.insertBefore(contentEl, buttonEl.nextSibling);
                        } else {
                            dropdownEl.appendChild(contentEl);
                    ...
                    if (itemTemplate && dropdownData.items.length > 0) {
                        dropdownData.items.forEach(item => {
                            ...
                            // Update location field
                            const locationEl = clone.querySelector('.location_connectivity_value');
                            if (locationEl) {
                                locationEl.setAttribute('data-field', item.valueField);
                                locationEl.textContent = item.value || '';
                            }
                            ...
                            // Update distance/time field
                            const timeEl = clone.querySelector('.location_connectivity_distance');
                            if (timeEl) {
                                timeEl.setAttribute('data-field', item.distanceField);
                                timeEl.textContent = item.distance || '';
                            }
                ...
                // Ensure map-cta button is at the bottom of map-right container
                const mapCtaButton = accordionWrapper.querySelector('.AnimatePrice');
                if (mapCtaButton) {
                    // Remove it from current position
                    mapCtaButton.remove();
                    // Append it to the bottom
                    accordionWrapper.appendChild(mapCtaButton);
                }
    ...
    function updateImagePaths(doc) {
        ...
        if (qrContainer || aboutSection || footersection) {
            ...
            // Priority 1: Find all QR images with data-field="qr" but ONLY in footer-qr-item (exclude RERA panel)
            const allDataFieldQr = doc.querySelectorAll('img[data-field="qr"]');
            // Filter to only include footer QR images (exclude RERA panel QR)
            const dataFieldQr = Array.from(allDataFieldQr).filter(img => {
                // Only include QR images in footer-qr-item containers, exclude RERA panel
                const isFooterQr = img.closest('.footer-qr-item');
                const isReraPanel = img.closest('.qr-section') && !isFooterQr;
                return isFooterQr && !isReraPanel;
            });
            
            if (dataFieldQr.length > 0 && hasUploads) {
                // Update existing footer QR images
                ...
                // Create additional QR images if we have more uploads than existing footer images
                if (qrUploads.length > dataFieldQr.length) {
                    const templateImg = dataFieldQr[0]?.cloneNode(true);
                    const baseAlt = templateImg?.getAttribute('alt') || 'QR Code';
                    
                    // Check if we're using footer-qr-item structure (template-5)
                    const footerQrGrid = footersection?.querySelector('.footer-qr-grid');
                    
                    // Check if we're using rera-img structure (template-3)
                    const existingReraContainer = dataFieldQr[0]?.closest('.rera-img');
                    const rowContainer = aboutSection?.querySelector('.row.mob-center, .row');
                    
                    if (footerQrGrid) {
                        // Template-5 structure: create new footer-qr-item containers in footer-qr-grid
                        for (let i = dataFieldQr.length; i < qrUploads.length; i++) {
                            const footerQrItem = doc.createElement('div');
                            footerQrItem.className = 'footer-qr-item';
                            
                            const qrImg = templateImg?.cloneNode(true) || doc.createElement('img');
                            const targetPath = getImageTargetPath(imageMapping.qr, i);
                            if (targetPath) {
                                qrImg.src = targetPath;
                                qrImg.setAttribute('src', targetPath);
                                qrImg.setAttribute('data-src', targetPath);
                            }
                            qrImg.alt = baseAlt.replace(/\d+$/, '') + ` ${i + 1}` || `QR Code ${i + 1}`;
                            qrImg.setAttribute('data-field', 'qr');
                            qrImg.setAttribute('loading', 'lazy');
                            qrImg.className = templateImg?.className || 'footer-qr-code';
                            if (templateImg?.hasAttribute('width')) {
                                qrImg.setAttribute('width', templateImg.getAttribute('width'));
                            }
                            if (templateImg?.hasAttribute('height')) {
                                qrImg.setAttribute('height', templateImg.getAttribute('height'));
                            }
                            qrImg.style.display = '';
                            qrImg.removeAttribute('style');
                            
                            footerQrItem.appendChild(qrImg);
                            footerQrGrid.appendChild(footerQrItem);
                        }
                    } else if (existingReraContainer && rowContainer) {
                        // Template-3 structure: create new rera-img containers in row
                        for (let i = dataFieldQr.length; i < qrUploads.length; i++) {
                            const colDiv = doc.createElement('div');
                            colDiv.className = 'col-lg-2 col-sm-3';
                            const reraContainer = doc.createElement('div');
                            reraContainer.className = 'rera-img';
                        ...
                        // Template-1, template-2 structure: append directly to qrContainer
                        for (let i = dataFieldQr.length; i < qrUploads.length; i++) {
                            const clone = templateImg?.cloneNode(true) || qrContainer.querySelector('img')?.cloneNode(true) || doc.createElement('img');
                            clone.style.display = '';
                            clone.removeAttribute('style');
                            
                            const targetPath = getImageTargetPath(imageMapping.qr, i) || clone.getAttribute('src') || '';
                            if (targetPath) {
                                clone.setAttribute('src', targetPath);
                            }
                            clone.setAttribute('alt', `${baseAlt} ${i + 1}`);
                            clone.setAttribute('data-field', 'qr');
                            clone.setAttribute('loading', 'lazy');
                            if (templateImg?.hasAttribute('height')) {
                                clone.setAttribute('height', templateImg.getAttribute('height'));
                            }
                            // Add spacing between QR images
                            if (i > 0) {
            ...
            if (dataFieldQr.length === 0) {
                const reraImgContainers = (qrContainer || aboutSection || footersection).querySelectorAll('.rera-img');
    ...
    function showTemplatePreview(imageSrc, templateName) {
        console.log(imageSrc,"template name = ",templateName)
        const temp = imageSrc.split("/")[0];
        console.log(imageSrc,"template name = ",templateName,"temp = ",temp);
